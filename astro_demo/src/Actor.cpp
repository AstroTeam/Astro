...
Actor::Actor(int x, int y, int ch, const char *name, const TCODColor &col):
	x(x),y(y),ch(ch),str(5),dex(3),intel(3),vit(5),totalDex(3), totalIntel(3),col(col),name(name),race("Human"),role("Marine"),job("Infantry"), blocks(true), smashable(false), oozing(false), susceptible(false), sort(0), attacker(NULL),destructible(NULL),ai(NULL),
	pickable(NULL), container(NULL) {
}

Actor::Actor(int x, int y, int ch, const char *name, const char *race, const char *role, const char *job, const TCODColor &col):
	x(x),y(y),ch(ch),str(5),dex(2),intel(3),vit(5),totalDex(3), totalIntel(3),col(col),name(name),race(race),role(role),job(job), blocks(true), smashable(false), oozing(false), susceptible(false), sort(0), attacker(NULL),destructible(NULL),ai(NULL),
	pickable(NULL), container(NULL) {
}
