...
void Map::addMonster(int x, int y) {
	...
	else
	{
		...
		infectedNCOChance += 5*(level-1); //increment infectedCrewMemChance by 5% each level
		infectedOfficerChance += 3*(level-1); //increment infectedCrewMemChance by 3% each level
		sporeCreatureChance += 2*(level-1); //increment infectedCrewMemChance by 2% each level
	}
